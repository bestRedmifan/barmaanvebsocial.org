...
function getSubject(post) {
  return String(post.subject ?? post.title ?? post.name ?? post.caption ?? "Untitled");
}
function getCaption(post) {
  if (!post || post.caption === undefined || post.caption === null) return "";
  return String(post.caption);
}
function getPostText(post) {
  if (!post) return "";
  if (post.text !== undefined && post.text !== null) return String(post.text);
  if (post.content !== undefined && post.content !== null) return String(post.content);
  if (post.file !== undefined && post.file !== null && String(post.type || "text").toLowerCase() === "text") return String(post.file);
  return "";
}
function isBlocked(post) {
  return Boolean(post && (post.blocked === true || post.blocked === "true"));
}
function getPostCode(post) {
  if (!post) return "";
  if (post.code !== undefined && post.code !== null) return String(post.code);
  if (post.password !== undefined && post.password !== null) return String(post.password);
  return "";
}
...
function askPostCode(post) {
  ...
  if (String(entered) === getPostCode(post)) {
    setUnlocked(post);
    return true;
  }
...
function makeCaption(post) {
  ...
  const node = el("div", "post-caption");
...
function renderBlocked() {
  ...
  box.appendChild(makeText("blocked"));
  return box;
}
...
function getCorrectQuizIndex(post) {
  const options = Array.isArray(post.options) ? post.options : [];
  if (post.correctAnswer !== undefined && post.correctAnswer !== null) {
    return options.findIndex(option => String(option) === String(post.correctAnswer));
  }
  if (post.correct !== undefined && post.correct !== null) {
...
function renderPollQuiz(post) {
  const box = el("div", "post-body");
  const options = Array.isArray(post.options) ? post.options : [];
  const correct = getCorrectQuizIndex(post);
...
function renderBody(post) {
  ...
  if (type === "photo" || type === "image") {
    const sources = Array.isArray(post.images) ? post.images : [post.image || post.src || post.file || post.url];
    ...
    box.appendChild(makeVideo(post.video || post.src || post.file || post.url));
  } else if (type === "file" || type === "download") {
    box.appendChild(makeDownload(post));
  } else if (type === "poll_original") {
    const poll = renderPollOriginal(post);
    if (caption) poll.prepend(caption);
    return poll;
  } else if (type === "poll_quiz") {
    const quiz = renderPollQuiz(post);
    if (caption) quiz.prepend(caption);
    return quiz;
  } else {
    box.appendChild(makeText(getPostText(post)));
  }
  return box;
}
function renderComments(post) {
  ...
  const list = getJSONComments(post);
  if (!list.length) {
    panel.appendChild(el("div", "no-comments", "No comments yet."));
  } else {
    list.forEach(comment => {
      panel.appendChild(el("div", "comment-item", comment));
    });
  }
  return panel;
}
...
function renderPost(post) {
  ...
  if (!isBlocked(post) && !(hasCode(post) && !isUnlocked(post))) {
...
function getSavedPostList() {
  ...
  const posts = Array.isArray(window.barmaanPosts) ? window.barmaanPosts : [];
  return posts.filter(post => saved.includes(getPostKey(post)));
}
...
// JSON comments are read-only.
// No comment input is created.
// No comment send button is created.
// Comments come only from posts.json.
// New comments are not stored in localStorage.
// Existing localStorage comment data is intentionally ignored.
// The C button only opens comments from posts.json.
// The caption field is read from posts.json.
// The code field is used as the protected-content password.
// The blocked field controls the blocked message.
// blocked true displays exactly: blocked.
// blocked false allows normal rendering.
// poll_original uses the options array from JSON.
// poll_quiz uses correctAnswer from JSON.
// text posts use the file field as their text.
// shorts posts use the file field as the video source.
// photo posts use the file field as the image source.
// file posts use the file field as the download source.
// The S button continues to use localStorage.
// Saved Subjects continues to use localStorage.
// Like state continues to use localStorage.
// Poll answers continue to use localStorage.
// Unlock state continues to use localStorage.
// Anti-Lag chunk rendering remains enabled.
// Fetch timeout remains enabled.
// Retry remains enabled.
// AbortController remains enabled.
// DOMContentLoaded remains enabled.
// window.barmaanReloadPosts remains enabled.
// The JSON array format remains supported.
// The JSON object with posts remains supported.
// IDs remain the primary post keys.
// File names remain fallback post keys.
// Subject names remain fallback post keys.
// Captions remain fallback titles.
// The existing CSS class names remain compatible.
// post remains the main card class.
// post-title remains the title class.
// post-body remains the body class.
// subject-container remains the body wrapper.
// social-controls remains the social control wrapper.
// heart-button remains the like button.
// comment-button remains the comments button.
// s-button remains the saved button.
// comments-panel remains the comments panel.
// comments-title remains the comments heading.
// comment-item remains each JSON comment.
// no-comments remains the empty comments message.
// post-caption remains the caption element.
// poll-option remains each poll option.
// quiz-result remains the quiz result.
// correct remains the correct quiz class.
// wrong remains the wrong quiz class.
// sel
