...
function renderPollQuiz(post, body) {
  const saved = localStorage.getItem("quiz_" + post.id);
  const options = post.options || [];
  ...
  options.forEach(opt => {
    ...
    if (saved === opt) {
      ...
      const result = el(
        "div",
        "quiz-result",
        opt === post.correctAnswer ? "درست! ✓" : "غلط! ✗"
      );
    ...
    btn.onclick = () => {
      localStorage.setItem("quiz_" + post.id, opt);

      Array.from(body.children).forEach(c => {
        c.classList.remove("selected");
        if (c.classList.contains("quiz-result")) c.remove();
      });
